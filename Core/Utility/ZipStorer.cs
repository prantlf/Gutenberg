...
namespace Gutenberg
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        /// <summary>
        /// Determines whether the specified file is a valid zip archive.
        /// </summary>
        /// <param name="_filename">The name of the file.</param>
        /// <returns>
        ///   <c>true</c> if the specified file is zip; otherwise, <c>false</c>.
        /// </returns>
        public static bool IsZip(string _filename) {
            Stream stream = new FileStream(_filename, FileMode.Open, FileAccess.Read);
            return IsZip(stream);
        }
        /// <summary>
        /// Determines whether the stream input contains a valid zip archive.
        /// </summary>
        /// <param name="_filename">The stream content.</param>
        /// <returns>
        ///   <c>true</c> if the stream input contains zip; otherwise, <c>false</c>.
        /// </returns>
        public static bool IsZip(Stream _stream) {
            if (!_stream.CanSeek)
                throw new InvalidOperationException("Stream cannot seek");

            if (_stream.Length < 22)
                return false;

            _stream.Seek(-17, SeekOrigin.End);
